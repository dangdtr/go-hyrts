...
//=======
//		if val, exist := versionDiff.GetCMs()[key]; exist && (val == parts[1]) {
//			return true, valDeps
//		}
//		if val, exist := versionDiff.GetAMs()[key]; exist && (val == parts[1]) {
//			return true, valDeps
//		}
//		if val, exist := versionDiff.GetDMs()[key]; exist && (val == parts[1]) {
//>>>>>>> Stashed changes
//			return true, valDeps
//		}
//	}
...
func isAffectedByChange(versionDiff diff.VersionDiff, depsMap map[string]string, covType string) (bool, []string) {
	...
	for key, valDeps := range depsMap {
		parts := strings.Split(key, ":")
		for _, v := range versionDiff.GetCMs() {
			if parts[1] == v {
				rs = append(rs, valDeps)
			}
		}

		for _, v := range versionDiff.GetAMs() {
			if parts[1] == v {
				rs = append(rs, valDeps)
			}
		}

		for _, v := range versionDiff.GetDMs() {
			if parts[1] == v {
				rs = append(rs, valDeps)
			}
		}

		for k := range versionDiff.GetAddedFiles() {
			if parts[0] == k {
				rs = append(rs, valDeps)
			}
		}

		for k := range versionDiff.GetDeletedFiles() {
			if parts[0] == k {
				rs = append(rs, valDeps)
			}
		}
